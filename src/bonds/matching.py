"""Universe-aware, no-look-ahead matching and fund-level bond metrics.

Ported from the bond pilot's matching module. The Parquet reader and secure
filesystem capability plumbing that fed ``ObservationIndex.build`` are replaced
by :meth:`ObservationIndex.from_rows`, an in-memory row source; the ephemeral
SQLite in-memory as-of index and the full ``MatchState`` precedence are
preserved verbatim. The two lanes stay isolated: :meth:`latest_rows` is
informative only and is never accepted by the point-in-time :func:`match_holding`
(passing it in raises ``invalid_observation_index``). Duplicate rows sharing a
``(cusip, observation_date)`` key stay ``UNAVAILABLE_AMBIGUOUS`` — never an
arbitrary pick. ``is_144a`` is derived from ``db_type == 3`` only when the field
is present and integral.
"""
...
from .states import DebtState, FieldState, MatchState
...
def _date(value: object) -> date:
    if not isinstance(value, str):
        raise BondError("invalid_match_date")
    try:
        return date.fromisoformat(value)
    except ValueError as exc:
        raise BondError("invalid_match_date", {"value": value}) from exc
...
class ObservationIndex(AbstractContextManager["ObservationIndex"]):
    ...
    @classmethod
    def from_rows(
        cls, rows: Iterable[Mapping[str, object]], cohort_cusips: Iterable[object]
    ) -> "ObservationIndex":
        """Build the index from observed-panel rows (dicts), never a file.

        Each row carries the panel's columns: ``normalized_cusip9``,
        ``observation_date``, ``source_row_number`` and the optional
        ``pr``/``pr_state``/``ytm``/``db_type``/``db_type_state``/
        ``daily_key_state``/``observation_date_state``. Rows outside the cohort,
        with a non-``present`` date state, an unparseable date, or a non-integer
        source row number are dropped exactly as the pilot dropped them.
        """
        cohort = {
        ...
        try:
            ...
            for row in rows:
                cusip = normalize_cusip9(row.get("normalized_cusip9")).normalized_cusip9
                date_state = row.get("observation_date_state", FieldState.PRESENT.value)
                if cusip not in cohort or date_state != FieldState.PRESENT.value:
                    continue
                try:
                    observed_on = _date(row.get("observation_date")).isoformat()
                except BondError:
                    continue
                ...
                observation_rows.append(
                    (
                        ...
                        row.get("ytm"),
                        row.get("db_type"),
                        row.get("db_type_state", FieldState.INVALID.value),
                        row.get("daily_key_state"),
                    )
                )
            if observation_rows:
                connection.executemany("INSERT OR IGNORE INTO universe (cusip) VALUES (?)", universe_rows)
                connection.executemany("INSERT INTO observations VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", observation_rows)
            connection.commit()
...
def _require_debt_mapping(mapping: object) -> DebtMapping:
    if not isinstance(mapping, DebtMapping):
        raise BondError("invalid_debt_mapping")
    return mapping
...
def match_holding(
    ...
    if not isinstance(observations, ObservationIndex):
        raise BondError("invalid_observation_index")
    debt_mapping = _require_debt_mapping(debt_mapping)
...
_CATEGORY_TERMINAL_STATES = frozenset(
    {MatchState.INELIGIBLE_NON_DEBT, MatchState.AMBIGUOUS_CATEGORY, MatchState.MISSING_CATEGORY}
)
...
def _validated_debt_state(match: MatchResult, debt_mapping: DebtMapping) -> DebtState:
    ...
    if debt_state is not DebtState.DEBT_LIKE_ELIGIBLE:
        ...
        if match.state is not expected:
            raise BondError("inconsistent_category_state")
    elif match.state in _CATEGORY_TERMINAL_STATES:
        raise BondError("inconsistent_category_state")
    return debt_state


def validate_match_categories(
    matches: Iterable[MatchResult], debt_mapping: DebtMapping
) -> tuple[tuple[MatchResult, DebtState], ...]:
...
def compute_series_metrics(matches: Iterable[MatchResult], debt_mapping: DebtMapping) -> tuple[SeriesMetric, ...]:
    """Compute metrics only from category states revalidated against the mapping."""
    ...
    for (series_id, report_date), rows in groups.items():
        ...
        if len(lineages) != 1:
            raise BondError("mixed_series_lineage", {"series_id": series_id, "report_date": report_date})
