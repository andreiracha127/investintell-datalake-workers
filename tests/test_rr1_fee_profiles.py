...
def _fixture(cur):
    ...
    cur.execute("""CREATE TABLE rr1_effective_facts(
        ...
        document_id text, dimensions text, occurrence integer, fact_typed_projection jsonb,
...
def test_rr1_fee_profiles_preserve_class_context_canonical_and_original_tag_evidence():
    ...
    with psycopg.connect(DSN, autocommit=True) as conn, conn.cursor() as cur:
        ...
        _fact(cur, run_id, "ManagementFees", "0.35", class_id="C1", document="D1", raw_row_id=11)
        _fact(cur, run_id, "ManagementFees", "0.45", class_id="C2", document="D2", raw_row_id=12)
        _fact(cur, run_id, "DistributionAndService12b1Fees", "0", class_id="C1", document="D1", raw_row_id=13)
        ...
        assert cur.fetchall() == [
            ("S1", "C1", "D1", "distribution_12b1", "DistributionAndService12b1Fees", "rr/2025", 0, "available", "", "", 1, "A1"),
            ("S1", "C1", "D1", "management_fee", "ManagementFees", "rr/2025", Decimal("0.35"), "available", "", "", 1, "A1"),
            ("S1", "C2", "D2", "distribution_12b1", None, None, None, "unavailable", "", "", 1, "A1"),
            ("S1", "C2", "D2", "management_fee", "ManagementFees", "rr/2025", Decimal("0.45"), "available", "", "", 1, "A1"),
        ]
...
def test_rr1_fee_profiles_keep_missing_distinct_from_zero_and_degrade_invalid_value():
    ...
    with psycopg.connect(DSN, autocommit=True) as conn, conn.cursor() as cur:
        schema, run_id, publication_id = _fixture(cur)
        _fact(cur, run_id, "ManagementFees", "0", raw_row_id=1)
        _fact(cur, run_id, "OtherExpenses", None, raw_row_id=2)
...
def test_rr1_fee_profiles_fail_closed_on_conflicting_selected_canonical_fact():
    ...
    with psycopg.connect(DSN, autocommit=True) as conn, conn.cursor() as cur:
        schema, run_id, publication_id = _fixture(cur)
        _fact(cur, run_id, "ManagementFees", "0.35", raw_row_id=1)
        _fact(cur, run_id, "ManagementFees", "0.45", raw_row_id=2)
...
def test_rr1_fee_profiles_inherit_effective_winner_and_pin_immutable_build_identity():
    ...
    with psycopg.connect(DSN, autocommit=True) as conn, conn.cursor() as cur:
        ...
        _fact(cur, amended_run, "ManagementFees", "0.20", accession="AMEND", raw_row_id=9)
        ...
        _fact(cur, amended_run, "OtherExpenses", "0.10", raw_row_id=10)
...
def test_rr1_fee_profiles_are_immutable_after_validation_and_current_view_is_derived_only():
    ...
    with psycopg.connect(DSN, autocommit=True) as conn, conn.cursor() as cur:
        schema, run_id, publication_id = _fixture(cur)
        _fact(cur, run_id, "ManagementFees", "0.35")
        cur.execute("SELECT build_rr1_fee_profiles(%s,'2026-06-30')", (publication_id,))
...
def test_rr1_fee_profile_ddl_is_effective_only_and_current_view_never_reads_raw():
    ...
    for token in ("rr1_fee_profile_v1", "rr1_effective_facts", "input_fingerprint", "management_fee",
                  "DistributionAndService12b1Fees", "sec_derived_current_pointers"):
        assert token in ddl
