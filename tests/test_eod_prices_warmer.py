...
def test_fetch_rate_stays_within_the_shared_account_budget():
    ...
    That pinned the defect in place: 25 req/s is 90k req/h against a 10k req/h
    account ceiling, so the warmer drained the fleet's whole hourly budget in
    ~6min40s each morning and the regime workers running in the same rolling
    hour got only 429s. The invariant worth holding is the account's, not the
    sweep's.
    """
