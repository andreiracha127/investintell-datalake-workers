...
def test_frozen_config_hash_is_the_preregistered_literal() -> None:
    assert config_hash() == "1863d3d5fa3a0edf"
    assert FROZEN["clock"] == {
    ...
    assert FROZEN["distribution_rule"] == "rule_144a_and_reg_s_explicit_mapping_v1"
    assert FROZEN["distribution_series"] == ["rule_144a", "reg_s"]
