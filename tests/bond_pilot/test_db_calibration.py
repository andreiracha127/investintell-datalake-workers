...
@pytest.mark.parametrize("mode,pages,rows", [("calibration", 1, 1000), ("calibration", 5, 5000), ("first_bounded", 1, 2500), ("first_bounded", 20, 50000)])
def test_complete_checkpoint_rejects_unproven_exact_multiple(mode: str, pages: int, rows: int, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    evidence, approval, _, _ = _governance(tmp_path, monkeypatch)
    from src.bond_pilot import db_calibration as calibration
...
def test_exact_full_page_requires_empty_terminator_and_empty_cohort_is_one_page(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    reports = [("S1", date(2024, 3, 31), "pub-1", "acc-1")]
    initial = (("S1",), ("2024-03-31",), ("pub-1",), ("acc-1",), 1000)
    full_page = [_row(holding_id=f"h-{index:04}") for index in range(1000)]
    key = ("S1", "2024-03-31", "pub-1", "acc-1", "h-0999", "run-1", "instrument-1")
    resume = (("S1",), ("2024-03-31",), ("pub-1",), ("acc-1",), *key, 1000)
    connection = TranscriptConnection(_prefix(calibration, reports) + [(EXPLAIN_INITIAL_SQL, initial, _plan()), (INITIAL_PAGE_SQL, initial, full_page), (EXPLAIN_RESUME_SQL, resume, _plan(resume=True)), (RESUME_PAGE_SQL, resume, [])])
    result = run_v2_calibration(connection, evidence=evidence, approval=approval, series_ids=("S1",), mode="calibration", checkpoint_path=tmp_path / "full" / "checkpoint.json", run_id="run-1")
    assert result.pages == 2 and result.rows_read == 1000 and result.partial is False
    connection.assert_exhausted()
