...
BEGIN
    ...
    WITH positions AS (
        SELECT b.series_id,h.report_date,h.holding_id,h.signed_market_value,h.signed_pct_of_nav,
               ...
               CASE WHEN NULLIF(btrim(h.cusip),'') IS NOT NULL THEN 'CUSIP:' || btrim(h.cusip)
                    WHEN NULLIF(btrim(h.isin),'') IS NOT NULL THEN 'ISIN:' || btrim(h.isin)
                    WHEN NULLIF(btrim(h.issuer_lei),'') IS NOT NULL THEN 'LEI:' || btrim(h.issuer_lei) END AS security_key
        ...
        SELECT series_id,report_date,count(*)::integer AS position_count,
               ...
               sum(abs(signed_market_value)) FILTER (WHERE security_key IS NOT NULL) AS identifier_market_value,
        ...
        SELECT series_id,report_date,security_key,sum(abs(signed_market_value)) AS gross_weight
        FROM positions WHERE signed_market_value IS NOT NULL AND security_key IS NOT NULL
        GROUP BY series_id,report_date,security_key
        ...
        WHERE a.unknown_market_value_position_count=0 AND a.gross_market_value>0
          AND a.identifier_market_value=a.gross_market_value
        GROUP BY a.series_id,a.report_date
    ), computed AS (
        SELECT a.*,
    ...
    INSERT INTO nport_composition_features (
        ...
        issuer_category_market_value_coverage,payoff_profile_market_value_coverage,issuer_market_value_coverage,
    ...
    SELECT target_publication_id,source_publication_id,c.series_id,c.report_date,as_of_date,
           CASE WHEN c.unknown_market_value_position_count>0 OR c.gross_market_value IS NULL OR c.gross_market_value<=0
                          OR c.identifier_coverage IS NULL OR c.issuer_category_coverage IS NULL OR c.payoff_profile_coverage IS NULL
                          OR c.identifier_coverage<0.70 OR c.issuer_category_coverage<0.70 OR c.payoff_profile_coverage<0.70
                          OR c.report_age_days>180 THEN 'insufficient'
                WHEN c.identifier_coverage<0.90 OR c.issuer_category_coverage<0.90 OR c.payoff_profile_coverage<0.90 THEN 'degraded'
                ELSE 'certified' END,
           to_jsonb(array_remove(ARRAY[
              ...
              CASE WHEN c.security_hhi IS NULL AND c.identifier_coverage<1 THEN 'security_identifier_coverage_incomplete' END,
           ...
           jsonb_build_object('source_surface','sec_nport_holdings_v2','source_holdings_publication_id',source_publication_id,
                              'source_fields',jsonb_build_array('issuer_category','payoff_profile','cusip','isin','issuer_lei','signed_market_value','signed_pct_of_nav'),
           ...
           c.issuer_category_coverage,c.payoff_profile_coverage,c.issuer_coverage,c.top_5_share,c.top_10_share,c.issuer_hhi,
